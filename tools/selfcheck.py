...
检查项（实现位置见括号）
------------------------
[1]  文件长度：每个 ``.py`` 与 ``web/*.js`` 必须少于 400 行。 （selfcheck_structure）
...
[1] 是**唯一**同时扫前端的一项：``web/*.js`` 进不了 AST 类检查（[2][9][10] 都
按 Python 语法树扫），但长度与语言无关。纳入的代价是既有违规会立刻暴露 ——
门禁先诚实，拆分另算（见 selfcheck_structure 的说明）。
