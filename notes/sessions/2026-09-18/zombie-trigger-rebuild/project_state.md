...
## 推送速率对照：容量按**字节**算（可证伪预测 + 命中）

若 2.24 MB 是**缓冲容量**（字节量），则推送快一倍 ⇒ 填满时间减半、**吞掉的字节不变**；
推送慢一倍 ⇒ 填满推迟、短卡杀不死协程。实测（`old off`）：

| 推送 | 卡住 | 冻结起点 | 吞掉 | 协程退出 | 僵尸 | RC |
|---|---|---|---|---|---|---|
| 400ms | 15s | **t=11.5s** | +28 帧 ≈ **2243 KB** | t=12.5~13.0s | ≥80s | 0 |
| **200ms** | 9s | **t=5.5s** | +26 帧 ≈ **2083 KB** | t=6.5s | ≥80s | 0 |
| **800ms** | 15s | **未填满** | +29 帧 ≈ 2323 KB（全发出） | **未退出** | **无** | 1 |
...
* ⚠️ **200ms 这档有混淆项**：推送频率正好等于页面渲染上限（`maxFps: 5`），
  6s 那臂在 `t=12.5s` 出现**与"卡住"无关**的第二次停顿 ⇒ 该档只能用来证明
  "容量按字节算"，**不能用来精确定边界**。

## 指标语义（我读错过一次，已写进探针）

`sent` 在发送协程死掉**之后就永远不会再动** ⇒ 探针报的「最长冻结」在下半段自动等于
"冻结起点 → 采样窗结束"，**不等于**"被堵了这么久"。15s 臂逐拍时间线坐实：
`sent` 在 t=11.5s 停住 → t=12.5s 起 `dropped` 开始涨（队列满、消费者已死）→
t=13.0s `sender_done=[True]`（超时退出）→ 之后 `sent` 再没动过。

⇒ 真实量是 **冻结起点（= 缓冲填满）** 与 **协程退出（= 冻结起点 + `send_timeout_s`）**。
探针已单独打印这两个量，并**在 `best < 3` 时不再报"冻结起点"**
（那只是采样间隔造成的一拍相同 —— 800ms 那臂就这么误导过一次）。

## 没做到的

* **仍未在 KAI 的真实浏览器 + 真实盘中背压下验证** —— 触发条件现在能按需重建，
  但重建用的是 headless Chrome；KAI 那个标签页当时到底被什么卡住，仍然未知。
* **"堵住之后为什么还要 9~11s 才松开"—— 该条作废（我读错了指标）**：没有"松开"，
  协程在冻结后 ≈1.0~1.5s 就退出了，`sent` 从此不动。详见上节。
* **帧尺寸方向仍未测**：容量 2.24 MB 目前只在 400ms 与 200ms 两档（同一帧尺寸 82 KB）
  下一致；帧变大时填满更快，但容量是否仍 ≈2.2 MB **未验证**。
