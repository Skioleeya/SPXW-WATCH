...
的门槛。热力图每档只留虚值一侧（那是**展示**选择），只喂虚值一侧就是
``2 × heatmap_draw_rows_each_side`` 个点（当前 40）；两侧都喂是 80 个点。
热力图的行数与曲面拟合的数据需求**不是一回事** —— 本快照取的是
``store.refs()`` 里**全部**当前会话合约（订阅窗口 ±20 档 × 两侧 = 80 条），
与热力图下发的 40 行无关，改热力图的窗口半径**不会**改变本层的喂数。
