...
`sent` 一旦停住，在发送协程死掉**之后就永远不会再动** ⇒ 探针报的「最长冻结」在下半段
自动等于"冻结起点 → 采样窗结束"，**不等于**"被堵了这么久"。15s 臂的逐拍时间线坐实：

```
t=11.0s sent=31              ← 一直在正常发
t=11.5s sent=32              ← 缓冲满，sent 停住
t=12.5s sent=32 dropped=1    ← 队列开始丢，协程还活着
t=13.0s sender_done=[True]   ← 超时退出（冻结后 ≈1.5s ≈ send_timeout）
t=22.6s sent=32 dropped=25   ← 之后再没动过
```

⇒ 真实的两个量是 **冻结起点（= 缓冲填满）** 与 **协程退出（= 冻结起点 +
`send_timeout_s`）**。探针已单独打印这两个量，且**在 `best < 3` 时不再报"冻结起点"**
（那只是采样间隔造成的一拍相同，会被读成"真堵住了" —— 800ms 那臂就这么误导过一次）。

## 推送速率对照：容量按**字节**算，不按时间算（可证伪预测）

若 2.24 MB 是**缓冲容量**（字节），则推送快一倍 ⇒ 填满时间应减半、**吞掉的字节不变**；
推送慢一倍 ⇒ 填满推迟、短卡就杀不死协程。实测（`old off`）：

| 推送 | 卡住 | 冻结起点 | 吞掉 | 协程退出 | 僵尸 | RC |
|---|---|---|---|---|---|---|
| 400ms | 15s | **t=11.5s** | +28 帧 ≈ **2243 KB** | t=12.5~13.0s（间隔 1.0~1.5s） | ≥80s | 0 |
| **200ms** | 9s | **t=5.5s** | +26 帧 ≈ **2083 KB** | t=6.5s（间隔 1.0s） | ≥80s | 0 |
| **800ms** | 15s | **未填满** | +29 帧 ≈ 2323 KB（全发出去了） | **未退出** | **无** | 1 |

- **填满时间 11.5s → 5.5s，比值 2.09 ≈ 400/200 ⇒ 预测命中**；吞掉的字节几乎不变
  （2.24 → 2.08 MB）⇒ **容量是字节量，与推送速率无关**。
- 800ms 那臂：填满要 ≈23s ⇒ **卡 15s 造不出僵尸**（预测命中）⇒ 下限公式
  `填满 + send_timeout` 成立，只是"填满"随推送速率走。
- ⚠️ **200ms 这档有混淆项，不能用来精确定边界**：推送频率正好等于页面渲染上限
  （`maxFps: 5`），6s 那臂在 `t=12.5s` 出现了一次**与"卡住"无关**的第二次停顿
  ⇒ 该档只用来证明"容量按字节算"。
...
- `tmp/probe_zombie_browser.py`（**新建**，519 → **566 行**，gitignored）：自带后端
  （产品自己的 `WsBroadcaster` + `StaticHandler`）+ 真帧样本 + 真 Chrome + CDP 卡主线程
  + 页面侧事件记录。`old` 版后端运行时从 git 取，打印 sha256。
  ⚠️ **519 → 566 行全是补扫时改的**：① docstring 加卡住时长边界表；
  ② 短卡那臂的 FAIL 文案由「卡主线程**不是**可行的触发手法」改为「**本臂时长不够**」；
  ③ 新增 `[指标语义]` 一行（单独报"冻结起点 / 协程退出"），并**在 `best < 3` 时不再报
  "冻结起点"**（那只是采样间隔造成的一拍相同）。**判据逻辑与判定阈值一字未动**
  ⇒ 已认证的臂输出不变（`old off 15 400` 在最终字节上复跑仍 **8/8 RC=0**、
  `+28 帧 ≈ 2243 KB`，与之前逐字一致）。
...
- 补扫卡住时长：`old off {8,10,12,13,14,15} 400 on` →
  **8/10/12s 无僵尸（RC=1，属预期：时长不够）；13/14/15s 僵尸形成（8/8，RC=0）**
- 推送速率对照（可证伪预测）：`old off 9 200 on` → **8/8 RC=0**（冻结 t=5.5s、+26 帧 ≈ 2083 KB）；
  `old off 6 200 on` → RC=1（**被 `maxFps: 5` 混淆，不作边界依据**）；
  `old off 15 800 on` → **RC=1**（未填满、协程未退出 ⇒ 卡 15s 不够）
- 最终字节上复跑：`old off 12 400`（RC=1，堵 0.5s）、`old off 15 400`（8/8 RC=0）、
  `old off 15 800`（RC=1）
...
# 背压建立所需的量（臂 1，卡住期间）
sent 4 → 32（+28 帧 ≈ 2243 KB）· 冻结于 t=11.5s · 协程退出于 t=12.5s（间隔 1.0s）

# 僵尸形态（臂 1 末拍）
clients=1 sender_done=[True] sent=32 dropped=224 | page text='数据中断 79s' forces=0

# 补扫卡住时长边界（`old off 400ms`；日志 tmp/sweep_2026-09-18/block_*.log）
8s  → 冻结 0.0s  · sent 4 → 44（+40 帧）  · 无僵尸   RC=1
10s → 冻结 0.0s  · sent 4 → 49（+45 帧）  · 无僵尸   RC=1
12s → 冻结 0.5s  · sent 4 → 54（+50 帧）  · 无僵尸   RC=1   ← 堵了，但 < send_timeout 1.0s
13s → 冻结 9.0s  · sent 4 → 32（+28 帧）  · 僵尸 ≥80s RC=0
14s → 冻结 10.5s · sent 4 → 32（+28 帧）  · 僵尸 ≥80s RC=0
15s → 冻结 11.0s · sent 4 → 32（+28 帧）  · 僵尸 ≥80s RC=0

# 推送速率对照（容量是字节量？可证伪预测；日志 tmp/sweep_2026-09-18/push_*.log）
push=200ms block=9s  → 冻结于 t=5.5s · +26 帧 ≈ 2083 KB · 僵尸 ≥80s · 8/8 RC=0   ← 预测命中
push=400ms block=15s → 冻结于 t=11.5s · +28 帧 ≈ 2243 KB · 僵尸 ≥80s · 8/8 RC=0
push=800ms block=15s → 未填满 · +29 帧 ≈ 2323 KB（全发出）· 协程未退出 · 无僵尸 · RC=1
   （11.5 / 5.5 = 2.09 ≈ 400/200 ⇒ 填满时间随推送速率走、吞掉的字节不变）
...
- **仍未在 KAI 的真实浏览器 + 真实盘中背压下验证**。触发条件现在能按需重建，
  ...
  （浏览器侧无埋点这件事没有改变）。
- **"堵住之后为什么还要 9~11s 才松开"—— 该条已作废（是我读错了指标）**：
  根本没有"松开"。协程在冻结后 ≈1.0~1.5s（≈`send_timeout_s`）就退出了，`sent` 从此
  永不再动；探针那个「最长冻结」是"冻结起点 → 采样窗结束"。指标语义已写进探针与本节。
- **推送速率只测了 200 / 400 / 800ms 三档，且 200ms 那档被 `maxFps: 5` 混淆** ⇒
  "容量 2.24 MB"目前是"在 400ms 与 200ms 两档下一致"的结论，**帧尺寸方向仍未测**
  （帧变大 ⇒ 填满更快，但容量是否仍 ≈2.2 MB 未验证）。
