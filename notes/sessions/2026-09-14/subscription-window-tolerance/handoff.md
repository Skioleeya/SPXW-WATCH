...
—— 74 与任何真实取值都对不上，是凭空多出来的第二份真相。
⚠️ **当时推导用的公式 `4 × each_side + 1` 也是错的**（见文末「订正」）：正确口径是
`4 × each_side`。
...
- `README.md` — 工具清单加新回归；`±12 档 → 49 条` 订正为 `±20 档 → 80 条`（口径见「订正」）；
...
## 订正（2026-09-14 12:1x，服务重启后）

**本轮把订阅条数的公式写错了，实测推翻。**

- **错**：`4 × each_side + 1`（±12 → 49、±20 → 81）。
- **对**：`4 × each_side`（±12 → 48、±20 → **80**）。

**证据**

1. `ChainResolver.window()`（`acquisition/chain_resolver.py:180`）取
   `below[-side:] + above[:side]` ⇒ 窗口是 **`2 × side` 档，不含中心档**。
   × Put/Call 两个权利 = `4 × side`。现货由 `subscribe_spot()` 单独订阅，
   **不进 `SubscriptionManager.count`** ⇒ 不加 1。
2. 实盘 `订阅 N/92` 取值分布（`logs/spxw_swatch.log` 全量）：
   `48`（469 次，±12）、`72`（296 次，±18）、`80`（±20）、`74`/`24`（更早的窗口定义）。
   **全是 4 的整数倍，没有出现过 `4n+1`。**
3. 重启后流水线首帧：`帧 118 | 客户端 0 | 分片 80 | 现价 7626.23 | 订阅 80/92`。
