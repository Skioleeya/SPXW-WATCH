...
window.SWATCH_CONFIG = {
  ...
  skew: {
    ...
    /* Y 轴自由缩放（滚轮）。
       ------------------------------------------------------------------
       为什么需要它：Y 轴量程默认由 `axisRange()` 每帧按"可见列的极值 + 留白"
       自动重算。Skew 长期在 2–3 波动率点内小幅波动，自动量程会把这点波动
       拉满整个面板高度 —— 看不出形态；而量程一被拉开（比如开盘跳空），
       小波动又被压成一条直线。

       `yZoom` 让用户用滚轮自己定纵轴量程。**X 轴完全不动** —— Y 轴缩放走
       独立通道，不碰 dataZoom（那是 X 轴纵向的），所以时间范围恒定不变。

       缩放态一旦建立就**锁定**（不再被自动量程覆盖），双击复位回自动量程。
       ------------------------------------------------------------------ */
    yZoom: {
      enabled: true,
      /* 一次滚轮的缩放倍率。1.15 ⇒ 每格约 ±15%，几格就能明显进出。 */
      step: 1.15,
      /* 锁定后仍可用的最短 / 最长量程（波动率点）。
         下限防止缩到数值精度以下（1e-4 远小于 axisDecimals 的 1 位小数，
         再缩就只剩一根线在动）；上限防止一次滚轮把量程拉到看不出结构。 */
      minSpan: 0.05,
      maxSpan: 500,
      /* 缩放锚点：鼠标指针所在的 Y 值在缩放前后保持不动（"往哪儿滚就往哪儿缩"）。
         关掉则锚在量程中心 —— 手感更"整"，但远离中心的位置会跑。 */
